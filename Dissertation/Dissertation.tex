...
\hl{A finite state machine, or finite automata, is a computational model which is widely used to simulate sequential logic, based on ideas such as a system changing state due to its inputs.}

\subsubsection{States}
\hl{A state is a representation of the finite state machine's current status. Each state will have a transition into another state, such that a collection of states can become a fully working, possibly cyclical system.}
...
\hl{According to Timothy Nodine in his paper, "Speciation in NEAT",}\cite{42} \hl{speciation is the set of processes by which NEAT creates, maintains and uses several disjoint groups of similar genomes for guiding reproduction. This means that given a species, A,  of an initial population, NEAT will create new subspecies, B, C, D, etc. each generation based on the similar attributes of the agents/genomes. This is useful for examining the internal behaviour between agents, as well as refining the eventual output of the prisoner's dilemma simulation.} \\

\subsection{Experimentation}
Upon implementing the program (both the neural networks and finite state machine versions), some form of experimentation must be conducted in order to validate the correctness/robustness of the program. In order to do this, the following section will include details on how to test the program, such as:
...
\hl{In the program, this simply comes down to:}
...
On page 139 of Harrald's\cite{25} paper, it is stated that the fitness for a given player, A, is calculated as follows: \\

\centerline{\textit{f}($\alpha, \beta$) = -0.75$\alpha + 1.75\beta$ + 2.25}
\medspace

Where $\alpha$ and $\beta$ are the moves of players A and B respectively. This subsection will examine the results of the program using the same parameters, but with this fitness function instead.

\begin{figure}[H]
	\centering
		\includegraphics[width=0.75\textwidth]{ContinuousFitnessSpeciationOff}
		\caption{Neural Networks Continuous Fitness With No Speciation}
\end{figure}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.75\textwidth]{ContinuousFitnessSpeciationOn}
		\caption{Neural Networks Continuous Fitness With No Speciation}
\end{figure}
...
\subsubsection{Repeated Prisoner's Dilemma (Continuous Fitness)}

\subsubsection{Multiple Replicants}
